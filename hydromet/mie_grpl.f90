...
! parameter and backscatter for a given water content of graupel
! in [g/m^3]. Microphysical properties are specified in the
! data structure 'grpl'.
!
!                 Updated from mie-graup_bs.f by H. M. (May, 2009)
!
 Subroutine mie_grpl(freq,temp,iwc,Nc,ksca,asca,gsca,pbck)
   Use MCRPparameters
   Implicit none
!
! I/O parameters
...
! iwc:  graupel ice water content [g/m**3]
! Nc:   graupel number concentration [1/m**3]
...
!
! normalize PSD if grpl%normaliz = .true.
!
   If ( grpl%normaliz ) then
      norm=0.
      do i=0,imax
         rad=dr*(.5 + float(i))
         D = 2*rad
         num = psd_func ( grpl,iwc,temp,Nc,D )
         densp = grpl%paramset(1)
...
!
   do i=0,imax
      ...
      D = 2*rad
      num = psd_func ( grpl,iwc,temp,Nc,D ) / norm
!
...
!
!       calculate dielectric constant of graupel as
