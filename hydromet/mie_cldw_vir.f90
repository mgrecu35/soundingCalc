...
! of liquid cloud in [g/m^3]. Microphysical properties are 
! specified in the data structure 'cldw'. For use by the 
! visible/IR simulator.
!
!                                 created by H. M. (May, 2009)
!
 Subroutine mie_cldw_vir(wl,temp,lwc,Nc,cext,csca,dscs)
...
!
   real(8), parameter :: dr = 1.d-4 ! Radius increment [mm] for PSD integration
   real(8), parameter :: rmax = 5.d-2 ! Maximum radius [mm] for PSD integration
...
!
   If ( cldw%normaliz ) then
      ...
      do i=0,imax
         ...
         num = psd_func ( cldw,lwcs,temps,Ncs,Ds )
...
!
   do i=0,imax
      ...
      D = 2*rad
      lwcs = lwc ; temps = temp ; Ncs = Nc ; Ds = D
      num = psd_func ( cldw,lwcs,temps,Ncs,Ds ) / norm
!
