...
! of snow in [g/m^3]. Microphysical properties are 
! specified in the data structure 'snow'. For use by the 
! visible/IR simulator.
!
!                                 created by H. M. (May, 2009)
!
 Subroutine mie_snow_vir(wl,temp,iwc,Nc,cext,csca,dscs)
...
! iwc:  snow water content [g/m**3]
! Nc:   snow water number concentration [1/m**3]
...
!
! normalize PSD if snow%normaliz = .true.
!
   If ( snow%normaliz ) then
      ...
      do i=0,imax
         ...
         num = psd_func ( snow,iwcs,temps,Ncs,Ds )
...
!
   do i=0,imax
      ...
      D = 2*rad
      iwcs = iwc ; temps = temp ; Ncs = Nc ; Ds = D
      num = psd_func ( snow,iwcs,temps,Ncs,Ds ) / norm
!
