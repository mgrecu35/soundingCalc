...
! of graupel in [g/m^3]. Microphysical properties are 
! specified in the data structure 'grpl'. For use by the 
! visible/IR simulator.
!
!                                 created by H. M. (May, 2009)
!
 Subroutine mie_grpl_vir(wl,temp,iwc,Nc,cext,csca,dscs)
...
! iwc:  graupel water content [g/m**3]
! Nc:   graupel water number concentration [1/m**3]
...
!
   If ( grpl%normaliz ) then
      ...
      do i=0,imax
         ...
         num = psd_func ( grpl,iwcs,temps,Ncs,Ds )
...
!
   do i=0,imax
      ...
      D = 2*rad
      iwcs = iwc ; temps = temp ; Ncs = Nc ; Ds = D
      num = psd_func ( grpl,iwcs,temps,Ncs,Ds ) / norm
!
