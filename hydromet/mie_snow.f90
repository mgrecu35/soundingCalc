...
! parameter and backscatter for a given water content of snow
! in [g/m^3]. Microphysical properties are specified in the
! data structure 'snow'.
!
!                 Updated from mie-snow_bs.f by H. M. (May, 2009)
!
 Subroutine mie_snow(freq,temp,iwc,Nc,ksca,asca,gsca,pbck)
   Use MCRPparameters
   implicit none
!
! I/O parameters
...
! Nc:   snow number concentration [1/m**3]
...
!
   If ( snow%normaliz ) then
      ...
      do i=0,imax
         rad=dr*(.5 + float(i))
         D = 2*rad
         num = psd_func ( snow,iwc,temp,Nc,D )
...
!
   do i=0,imax
      ...
      D = 2*rad
      num = psd_func ( snow,iwc,temp,Nc,D ) / norm
!
