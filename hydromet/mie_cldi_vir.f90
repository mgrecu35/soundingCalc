...
! of ice cloud in [g/m^3]. Microphysical properties are 
! specified in the data structure 'cldi'. For use by the 
! visible/IR simulator.
!
!                                 created by H. M. (May, 2009)
!
 Subroutine mie_cldi_vir(wl,temp,iwc,Nc,cext,csca,dscs)
...
! Nc:   cloud ice water number concentration [1/m**3]
...
!
   real(8), parameter :: dr = 5.d-3 ! Radius increment [mm] for PSD integration
   real(8), parameter :: rmax = 2.5d0 ! Maximum radius [mm] for PSD integration
...
!
   If ( cldi%normaliz ) then
      ...
      do i=0,imax
         ...
         num = psd_func ( cldi,iwcs,temps,Ncs,Ds )
...
!
   do i=0,imax
      ...
      D = 2*rad
      iwcs = iwc ; temps = temp ; Ncs = Nc ; Ds = D
      num = psd_func ( cldi,iwcs,temps,Ncs,Ds ) / norm
!
